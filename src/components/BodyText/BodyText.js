"use client";
import React from "react";
import { lazy, Suspense } from "react";
import "./BodyText.scss";
import AdManagerSlot from "../AdvModule/AdManagerSlot";
// import { useGlobalLocation } from "../../shared/LocationContext.js";
import Zadusnice from "../Boxes/Zadusnice/Zadusnice.js";
import TodayBox from "../Boxes/TodayBox/TodayBox.js";
import OnNetworkVideo from "../AdvModule/OnNetworkVideo.js";
import { useRouteContext } from "../../shared/RouteProvider.js";
import { useScriptContext } from "../../shared/ScriptProvider.js";
const dynamicComponents = {
  Zadusnice,
  TodayBox,
};

const PostImage = lazy(
  () => import("../../containters/SinglePost/img/PostImage.js"),
);



export default function BodyText(props) {
  const { pathPart } = useRouteContext();
  const { cyr, cyrHtml } = useScriptContext();

  const setSubTitle = (item) =>
  item.subtitle ? (
    <h2 className="mainContent-subtitle">{cyr(`${item.subtitle}`)}</h2>
  ) : item.subtitle3 ? (
    <h3 className="mainContent-subtitle3">{cyr(`${item.subtitle3}`)}</h3>
  ) : null;
const setQuote = (item) => {
  if (item.textQuote) {
    return (
      <blockquote
        className="textQuote"
        dangerouslySetInnerHTML={{ __html: cyrHtml(item["textQuote"] )}}
      ></blockquote>
    );
  }
};
const setImg = (item) => {
  if (item.imgSrc) {
    return (
      <div className="bodyImg">
        <Suspense fallback={<div></div>}>
          <PostImage src={item.imgSrc[0]} alt={item.imgSrc[1]} />
        </Suspense>
        <figcaption className="mainContent-img_source">
          <span>{cyr(`Foto:`)}{" "}</span>
          <span dangerouslySetInnerHTML={{ __html: cyrHtml(item.imgSrc[1])}}></span>
        </figcaption>
      </div>
    );
  }
};
const setComponent = (item) => {
  if (item.component && dynamicComponents[item.component]) {
    const Component = dynamicComponents[item.component];
    return (
      <Suspense fallback={<div></div>}>
        <Component classes="horizontal" />
      </Suspense>
    );
  }
  return null;
};

  return (
    <div className="bodyText">
      {props.bodyText?.map((item, index) => {
        if (index === 0) {
          return (
            <React.Fragment key={index}>
              {setComponent(item)}
              {setSubTitle(item)}
              {/* <p dangerouslySetInnerHTML={{ __html: item["text"] }}></p> */}
              <p
                dangerouslySetInnerHTML={{ __html: cyrHtml(item["text"]) }}
              ></p>
              {setQuote(item)}
              {setImg(item)}

              {pathPart[1] !== "molitvenik" && pathPart[1] !== "" && (
                <OnNetworkVideo sid="NVNnLDAsMVd4" />
              )}
              {/* NVNnLDAsMVd4 */}
              {pathPart[1] !== "molitvenik" && pathPart[1] !== "" && (
                <div className="banner-wrapper">
                  <AdManagerSlot slotNumber={"div-gpt-ad-1750930023966-0"} />
                </div>
              )}
            </React.Fragment>
          );
        } else {
          return (
            <React.Fragment key={index}>
              {setComponent(item)}
              {setSubTitle(item)}
              <p
                dangerouslySetInnerHTML={{ __html: cyrHtml(item["text"]) }}
              ></p>
              {setImg(item)}
              {setQuote(item)}
              {/* {setComponent(item)} */}
            </React.Fragment>
          );
        }
      })}
    </div>
  );
}
