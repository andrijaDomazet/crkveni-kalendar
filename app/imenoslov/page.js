...
export function generateMetadata() {
  const article = findArticle("imenoslov");
  if (!article) return { title: "Imenoslov | crkveni-kalendar.net" };
  const title = article.title_2 || article.title;
  return {
    title,
    description: article.lead,
    alternates: { canonical: "/imenoslov/" },
    openGraph: {
      locale: "sr-RS",
      type: "article",
      title,
...
export default function ImenoslovPage() {
  return <SinglePost />;
}
