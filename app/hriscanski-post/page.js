...
export function generateMetadata() {
  const article = findArticle("hriscanski-post");
  if (!article) return { title: "Hrišćanski post | crkveni-kalendar.net" };
  const title = article.title_2 || article.title;
  return {
    title,
    description: article.lead,
    alternates: { canonical: "/hriscanski-post/" },
    openGraph: {
      locale: "sr-RS",
      type: "article",
      title,
...
export default function HriscanskiPostPage() {
  return <SinglePost />;
}
