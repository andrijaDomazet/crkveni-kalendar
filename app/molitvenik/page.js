...
export function generateMetadata() {
  const article = findArticle("molitvenik");
  if (!article) return { title: "Molitvenik | crkveni-kalendar.net" };
  const title = article.title_2 || article.title;
  return {
    title,
    description: article.lead,
    alternates: { canonical: "/molitvenik/" },
    openGraph: {
      locale: "sr-RS",
      type: "article",
      title,
...
export default function MolitvenikPage() {
  return <SinglePost />;
}
