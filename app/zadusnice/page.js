...
export function generateMetadata() {
  const article = findArticle("zadusnice");
  if (!article) return { title: "Zadušnice | crkveni-kalendar.net" };
  const title = article.title_2 || article.title;
  return {
    title,
    description: article.lead,
    alternates: { canonical: "/zadusnice/" },
    openGraph: {
      locale: "sr-RS",
      type: "article",
      title,
...
export default function ZadusnicePage() {
  return <SinglePost />;
}
